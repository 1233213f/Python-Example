...
def db_store(title, content, time, keyword, category, href, artificial):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO scrapy (title,content,time,keywords,category,href,artificial) VALUES (\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\")",
            (title, content, time, keyword, category, href, artificial))
        cur.connection.commit()
    except ProgrammingError as e:
        if 1146 == e.args[0]:
            cur.execute(
                "CREATE TABLE IF NOT EXISTS scrapy (Id int primary key auto_increment,title varchar(255),content longtext,time varchar(255),keywords varchar(255),category varchar(255),href varchar(255),artificial varchar(255))")
